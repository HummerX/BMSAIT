...
unsigned long timePauseExt=0;  //pause when poti was moved
unsigned long timePauseInt=0;  //pause when BMS trim got moved
unsigned long time_motormove=0;  //time since start of motor movement
...
bool CheckDirection(byte motor,bool richtung,bool recursive)
{
  ...
  else
  {
    ...
    if (tmp>oldPos+BUFFER) 
    {
      //pot readout increased. motor and pot work in opposite directions
      motorPoti[motor].dir=false;
      return false;
    }  
    else if (tmp<oldPos-BUFFER) 
    ...
    {
      if (recursive) 
        return true; //if this is the second attempt to check the sync, return an error (true).
      else
      {
        //no good signal so far. Lets try the other direction
        if (CheckDirection(motor,!richtung,true))   
...
void CheckExternalMovement(byte mp)
{
  ...
  if (abs(motorPoti[mp].trimPos_ext-motorPoti[mp].trimPos_int)>BUFFER)  
  {
    ...
    bool MotDirection=motorPoti[mp].dir;
    if (motorPoti[mp].trimPos_ext>motorPoti[mp].trimPos_int){MotDirection=!MotDirection;}
