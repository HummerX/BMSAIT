...
bool SetBlinkOn = false;

LedControl LEDController=LedControl(0,0,0, 1); //empty call of the LEDControl class

#define LEDM_CLK 3   //PIN "Clock" for the SPI connection of the MAX7219 LED Control
#define LEDM_CS  4    //PIN "Cable Select" for the SPI connection of the MAX7219 LED Control
#define LEDM_DIN 2  //PIN "Data In" for the SPI connection of the MAX7219 LED Control
#define LEDM_BRIGHTNESS 10  //sets the intensity of the LEDs (0..15)
...
void UpdateLEDMatrix(byte p)
{
  ...
  if (SetBlinkOn && (millis()>LEDMatrixTimer+BLINKSPEED))
