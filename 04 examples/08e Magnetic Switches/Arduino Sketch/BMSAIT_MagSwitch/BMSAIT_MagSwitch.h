...
{  
 ...
 //sum up all encoders and do an initial sweep once for each encoder
  ...
  {
    for (byte lauf=0;lauf<magSwitchCount;lauf++)   //run through data container and look for servos that are defined twice (i.e. autopilot)
    {
      boolean found=false;
      ...
      if (!found)
      {
        MagPins[pos]=lauf;  //each Servo-Pin will be memorized once
        pos++;   
      }
    }
  } 
  
  for (byte id : MagPins) //each identified servo PIN will be activated at the same time
  {
    if (id!=0)
    {
     magSwitchServo[id].attach(magSwitchData[id].servoPin); 
