...
{ //WritePIN   Adress1+0    DataPINs
 ...
 // , {  A1,      A2, A3,     2, 3, 4, 5, 6, 7, 8     } //optional additional modules (adress-Pins and data-Pins can be shared between multiple modules)
};
...
{
  digitalWrite(slxModules[SLDevice].WR_PIN, HIGH);   //pause write commands
  delayMicroseconds(200);
  for (byte SLZiffer = 0; SLZiffer < ZifferAnz; SLZiffer++)
  {
    digitalWrite(slxModules[SLDevice].AdressPIN1, bitRead(ZifferStart+SLZiffer, 0));  //select figure
    digitalWrite(slxModules[SLDevice].AdressPIN2, bitRead(ZifferStart+SLZiffer, 1));  //select figure 
    char SLZahl=fig[ZifferAnz-SLZiffer-1];

    for (byte dataBitPos = 0; dataBitPos < 7; dataBitPos++)
    {
      digitalWrite(slxModules[SLDevice].DataPIN[dataBitPos], bitRead(SLZahl, dataBitPos)); //set data pins
    }
    delayMicroseconds(200);
